...
class Helpers {
	...
	public static function locate_template( string $tpl ) {
		...
		$path = apply_filters( 'beapi_helpers_locate_template_templates', [ 'views/' . BEAPI_ACF_PALETTE_VIEWS_FOLDER_NAME . '/' . $tpl . '.php' ], $tpl, __NAMESPACE__ );
		...
		// Locate on the files
		if ( is_file( BEAPI_ACF_PALETTE_DIR . 'views/' . $tpl . '.php' ) ) {// Use builtin template
			return ( BEAPI_ACF_PALETTE_DIR . 'views/' . $tpl . '.php' );
		}
